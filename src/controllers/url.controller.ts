...
export class UrlController {
   ...
   */
  static getWebsiteInfo = async (req: Request, res: Response): Promise<void> => {
    try {
      const { url } = req.body;
      ...
      const iconLinks = $('link[rel="icon"], link[rel="shortcut icon"], link[rel="apple-touch-icon"]');
      ...
      res.status(500).json({
        ...
        description: '',
        url: '',
        message: '获取网站信息失败: ' + (error as Error).message
      });
    }
  };
}
